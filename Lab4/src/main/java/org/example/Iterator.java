...
public class Iterator<K> {
    ...
    public List<Vertex<K>> reverseVertexList(){
        for(int i=0;i!=Math.floor(vertexList.size()/2);++i){
            swap(vertexList.get(i),vertexList.get(vertexList.size()-i));
        }
        return vertexList;
    }

    public List<Edge<K>> reverseEdgeList(){
        for(int i=0;i!=Math.floor(edgeList.size()/2);++i){
            swap(edgeList.get(i),edgeList.get(edgeList.size()-i));
        }
        return edgeList;
    }
